...
object OpenCLScalaTest3 {
  object CL {
    ...
    trait Translator[A,B] {
      def size(a: A): Int
      def align: Int
      def copyIn(dev: Device, a: A): B
      def copyOut(dev: Device, b: B): A
    }

    implicit object IT extends Translator[Int,Int] {
      def size(a: Int) = 4
      def align = 4
      def copyIn(dev: Device, a: Int) = a
      def copyOut(dev: Device, b: Int) = b
    }

    implicit object FT extends Translator[Float,Float] {
      def size(a: Float) = 4
      def align = 4
      def copyIn(dev: Device, a: Float) = a
      def copyOut(dev: Device, b: Float) = b
    }

    implicit object FAT extends Translator[Array[Float],Buffer[Float]] {
...
println("readEvent = " + readEvent)
  ...
  class Device(val context: SCLContext) {
    def compile1[A: ClassManifest,B: ClassManifest](name: String, src: String): Kernel1[Buffer[A],Buffer[B]] = {
    ...
    def spawn[A,B](totalNumberOfItems: Int, numberOfItemsPerGroup: Int)(k: RunnableKernel1[A,B]) = k.run(this, totalNumberOfItems, numberOfItemsPerGroup)
  }
  ...
  def main(args: Array[String])  = {
    ...
    // how to encode the annotation?
    // a: Array[Float] @global
    // a: Array[Float] @mem("global")
    // case class Global[T](a: T)

    val kernel = CL.gpu.compile1[Float, Float]("copyVec", src)

    val wrapper = new Kernel1[Array[Float], Array[Float]] {
      def apply(input: Array[Float]) = new RunnableKernel1[Array[Float],Array[Float]] {
        def run(dev: Device, totalNumberOfItems: Int, numberOfItemsPerGroup: Int) = {
          val trans = implicitly[Translator[Array[Float], Buffer[Float]]]

          val bufIn = trans.copyIn(dev, input)

          val f = dev.spawn(totalNumberOfItems, numberOfItemsPerGroup) { kernel(bufIn) }

          new Future[Array[Float]] {
            def force = {
              val result = f.force
              trans.copyOut(dev, result)
            }
          }
        }
      }
    ...
    val result = CL.gpu.spawn(dataSize) { wrapper(a) }
