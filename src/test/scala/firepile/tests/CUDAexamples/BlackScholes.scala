...
$if(0) {
    ...
    def EXP(a:Float)=native_exp(a)
    def LOG(a:Float)=native_log(a)
    def SQRT(a:Float)=native_sqrt(a)
}
$else
{
    def EXP(a:Float)=exp(a)
    def LOG(a:Float)=log(a)
    def SQRT(a:Float)=sqrt(a)
}
...
def BlackScholesBody( call: GlobalFloatP, put: GlobalFloatP, S: Float, X: Float, T: Float, R: Float , V: Float) ={
...
def GPU_BlackScholes( d_Call: GlobalFloatP, d_Put: GlobalFloatP, d_S: GlobalFloatP, d_X: GlobalFloatP, d_T: GlobalFloatP, 
                    R: Float, V: Float, optN: UInt ) ={
   
   for(var opt:UInt <- getGlobalId(0) until optN){
        BlackScholesBody( d_Call(opt), d_Put(opt), d_S(opt), d_X(opt), d_T(opt), R, V)
        opt+=getGlobalId(0)
   }
  }
