...
$if(1){
 ...
 implicit def scan1Inclusive(idata: UInt , l_Data: LocalUIntP, size: UInt ) : UInt  ={
        
        val pos: UInt = 2 * getLocalId(0) - (getLocalId(0) & (size - 1))
        l_Data(os) = 0
        pos += size
        l_Data(pos) = idata
        
        for(var offset:UInt <- 1 until size ){
        
            localMem.barrier
            var t:UInt = l_Data(pos) + l_Data(pos - offset)
    ...
    implicit def scan1Exclusive(idata: UInt , l_Data: LocalUIntP, size: UInt ) : UInt  ={
        scan1Inclusive(idata, l_Data, size) - idata
    }
...
def LOG2_WARP_SIZE = 5U
def WARP_SIZE = 1U << LOG2_WARP_SIZE
     
      implicit warpScanInclusive(idata: UInt , l_Data: LocalUIntP, size: UInt){
    ...
    implicit warpScanExclusive(idata: UInt , l_Data: LocalUIntP, size: UInt){
          warpScanInclusive(idata, l_Data, size) - idata
    }

    implicit scan1Inclusive(idata: UInt , l_Data: LocalUIntP, size: UInt){
    
        if(size > WARP_SIZE){
        
            val warpResult: UInt = warpScanInclusive(idata, l_Data, WARP_SIZE)

            localMem.barrier
     
            if( (getLocalId(0) & (WARP_SIZE - 1)) == (WARP_SIZE - 1) )
                l_Data(getLocalId(0) >> LOG2_WARP_SIZE) = warpResult

            localMem.barrier
         
            if( getLocalId(0) < (WORKGROUP_SIZE / WARP_SIZE) ){
         
                val vl: UInt = l_Data(getLocalId(0))
                l_Data(get_local_id(0)) = warpScanExclusive(vl, l_Data, size >> LOG2_WARP_SIZE)
            }

            localMem.barrier
            return warpResult + l_Data(getLocalId(0) >> LOG2_WARP_SIZE)
        }else
            return warpScanInclusive(idata, l_Data, size)
     }

    implicit scan1Exclusive(idata: UInt , l_Data: LocalUIntP, size: UInt){
        scan1Inclusive(idata, l_Data, size) - idata
    }

}

implicit scan4Inclusive(data4: UInt4, l_Data: LocalUIntP,size: UInt): UInt4  ={
    data4.y += data4.x
    data4.z += data4.y
    data4.w += data4.z

    val vl: UInt4 = scan1Inclusive(data4.w, l_Data, size / 4) - data4.w

    data4 + vl
}

implicit scan4Exclusive(data4: UInt4, l_Data: LocalUIntP, size: UInt): UInt4  ={
    scan4Inclusive(data4, l_Data, size) - data4
}

//Scala Kernels 
val a: Attribute = (reqd_work_group_size(WORKGROUP_SIZE, 1, 1))

def GPU_scanExclusiveLocal1(a: Attribute)( d_Dst: GlobalUInt4, d_Src: GlobalUInt4, l_Data: LocalUIntP, size: UInt) ={
    val idata4: UInt4 = d_Src(getGlocalId(0))

    val odata4: UInt4  = scan4Exclusive(idata4, l_Data, size)

    d_Dst(getGlobalId(0)) = odata4
    
}

def GPU_scanExclusiveLocal2(a: Attribute)(d_Buf: GlobalUInt4P, d_Dst: GlobalUInt4P, d_Src: GlobalUint4P, l_Data: LocalUIntP, 
size: N, 
size: arrayLength) ={
    ...
    if(getGlobalId(0) < N)
    data =
        d_Dst((4 * WORKGROUP_SIZE - 1) + (4 * WORKGROUP_SIZE) * getGlobalId(0))+ 
        d_Sr((4 * WORKGROUP_SIZE - 1) + (4 * WORKGROUP_SIZE) * getGlobalId(0))

    val odata: UInt = scan1Exclusive(data, l_Data, arrayLength)

    if(getGlobalId(0) < N)
        d_Buf(getGlobalId(0)) = odata
}

def GPU_uniformUpdate(a: Attribute)(d_Data: GlobalUInt4, d_Buf: GlobalUInt ={

    val buf: Array[LocalUInt]=Array[LocalUInt](0)

    val data4: UInt4 = d_Data(getGlobalId(0))

    if(getLocalId(0) == 0)
        buf(0) = d_Buf(getGroupId(0))

    localMem.barrier
    
    data4 += (UInt)buf(0)
    d_Data(getGlobalId(0)) = data4
}
